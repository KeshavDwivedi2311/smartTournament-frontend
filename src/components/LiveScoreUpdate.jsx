...
const LiveScoreUpdate = ({ match, onClose, onRefresh }) => {
  ...
  const updateScore = async (team, increment) => {
    const newScores = {
      ...scores,
      [`${team}Score`]: Math.max(0, scores[`${team}Score`] + increment)
    };
    setScores(newScores);
    setError(null);
  ...
  const winner = getWinnerIndicator();

  return (
    <div className="bg-gradient-to-br from-gray-50 to-blue-50 border-t border-gray-200 rounded-b-lg p-3 sm:p-4 lg:p-6 mt-3">
